...
We copied the existing `bloglist_backend` into the Part 5 E2E testing project so that we could use a separate test environment.

We then installed the dependencies inside the copied backend:
...
This tells the application that we are running tests.
...
We also discovered that the server was actually listening on port `3003`, even though the `logger.info()` message was not showing.
...
Our first E2E test checks that the Bloglist application's front page can be opened and that the title is visible.
...
We checked test discovery with:
...
We checked the backend manually and confirmed that it worked.

We then used `curl` to check whether the frontend was actually reachable.
...
After starting the frontend, we discovered that `127.0.0.1` worked while `localhost` did not.

`localhost` normally points to the same computer as `127.0.0.1`, but on this machine they were not behaving the same way.
...
curl is a command-line tool for checking or communicating with a web address. A quick way to check whether a web server can be reached from your computer.
...
This helped us discover that localhost wasn't working correctly on our machine, while 127.0.0.1 was.
...
The biggest lesson from this setup was that an E2E test is testing the **whole application working together**, not just one piece.
...
When something goes wrong, check each piece separately instead of assuming the test itself is broken.
